...
class FirestoreOnuCache(private val context: Context? = null) {
    ...
    private val onusCollection = "onus_cache" // Updated collection name
    ...
    suspend fun getOnusFromCache(): List<OnuDetail>? {
        return try {
            ...
            snapshot.documents.mapNotNull { document ->
                try {
                    OnuDetail(
                        uniqueExternalId = document.getString("unique_external_id"),
                        sn = document.getString("sn") ?: "",
                        name = document.getString("name") ?: "",
                        oltId = document.getString("olt_id"),
                        oltName = document.getString("olt_name"),
                        board = document.getString("board"),
                        port = document.getString("port"),
                        onu = document.getString("onu"),
                        onuTypeId = document.getString("onu_type_id"),
                        onuTypeName = document.getString("onu_type_name"),
                        zoneId = document.getString("zone_id"),
                        zoneName = document.getString("zone_name"),
                        address = document.getString("address"),
                        odbName = document.getString("odb_name"),
                        mode = document.getString("mode"),
                        wanMode = document.getString("wan_mode"),
                        ipAddress = document.getString("ip_address"),
                        subnetMask = document.getString("subnet_mask"),
                        defaultGateway = document.getString("default_gateway"),
                        dns1 = document.getString("dns1"),
                        dns2 = document.getString("dns2"),
                        username = document.getString("username"),
                        password = document.getString("password"),
                        catv = document.getString("catv"),
                        administrativeStatus = document.getString("administrative_status"),
                        servicePorts = emptyList(),
                        phoneNumber = document.getString("phone_number"),
                        rxPower = document.getDouble("rx_power"),
                        txPower = document.getDouble("tx_power"),
                        lastSeen = document.getString("last_seen") ?: "Unknown",
                        distance = document.getLong("distance")?.toInt(),
                        model = document.getString("onu_type_name")
                    )
                } catch (e: Exception) {
            ...
            onus.chunked(100).forEach { batch ->
                ...
                batch.forEach { onuDetail ->
                    ...
                    val onuData = mapOf(
                        ...
                        "olt_id" to onuDetail.oltId,
                        "olt_name" to onuDetail.oltName,
                        "board" to onuDetail.board,
                        "port" to onuDetail.port,
                        "onu" to onuDetail.onu,
                        "onu_type_id" to onuDetail.onuTypeId,
                        "onu_type_name" to onuDetail.onuTypeName,
                        "zone_id" to onuDetail.zoneId,
                        "zone_name" to onuDetail.zoneName,
                        "address" to onuDetail.address,
                        "odb_name" to onuDetail.odbName,
                        "mode" to onuDetail.mode,
                        "wan_mode" to onuDetail.wanMode,
                        "ip_address" to onuDetail.ipAddress,
                        "subnet_mask" to onuDetail.subnetMask,
                        "default_gateway" to onuDetail.defaultGateway,
                        ...
                        "administrative_status" to onuDetail.administrativeStatus,
                        "phone_number" to onuDetail.phoneNumber,
                        "status" to onuDetail.status,
                        "rx_power" to onuDetail.rxPower,
                        "tx_power" to onuDetail.txPower,
                        "last_seen" to onuDetail.lastSeen,
